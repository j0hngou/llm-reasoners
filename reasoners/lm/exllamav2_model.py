...
class ExLlamaV2Model(LanguageModel):
    def __init__(self, model_dir, lora_dir, max_batch_size, max_new_tokens, max_seq_length, device='cuda:0',
        ...
        """
        Initializes an ExLlamaV2Model instance.

        Args:
            model_dir (str): Path to the directory containing the ExLlamaV2 model files.
        ...
        self.config = ExLlamaV2Config(model_dir)  # create config from config.json
        ...
        self.model = ExLlamaV2(self.config)  # create ExLlama instance and load the weights
        self.tokenizer = ExLlamaV2Tokenizer(self.config)  # create tokenizer from tokenizer model file

        self.cache = ExLlamaV2Cache(self.model, max_batch_size, lazy=True)
        self.model.load_autosplit(self.cache) # create cache for inference
        self.generator = ExLlamaV2BaseGenerator(self.model, self.tokenizer, self.cache)  # create generator
        self.lora = None
        ...
        if lora_dir is not None:
            ...
            self.lora = ExLlamaV2Lora(self.model, lora_config_path, lora_path)
    ...
    def generate(
        ...
        if max_length is not None:
            warnings.warn("max_length is not supported by ExLlamaV2Model for generation. Use max_new_tokens instead.")
        ...
        for start in range(0, len(inputs), self.max_batch_size):
            ...
            if output_log_probs:
                warnings.warn(
                    "output_log_probs is temporarily not supported now by ExLlamaV2Model. Please refere to exllama's code")
    ...
    def get_loglikelihood(self, prefix: str, contents: list[str], **kwargs) -> np.ndarray:
        ...
        prompts_tokens = self.tokenizer.encode(contents, add_bos=True, add_eos=False)
